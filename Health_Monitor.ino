...
// Try 0x27 first, if not working use 0x3F
LiquidCrystal_I2C lcd(0x27, 16, 2);

void setUpLcd(){
  Wire.begin(D2, D1);   // ✅ IMPORTANT for ESP8266 (SDA, SCL)

  lcd.init();           // ✅ correct for ESP8266
  lcd.backlight();
...
//hotspot config
const char* hotspot_name = "iota0507";
const char* hotspot_password = "iota0507";
...
struct{
	...
	int heartrate;
	String heartrate_class = "success";

	int humidity;
	String humidity_class = "success";

	int oxigen;
	String oxigen_class = "success";
...
String getDataJson(){
	...
	"\"heartrate\":\""+String(dataPacket.heartrate)+"\",\"heartrate_class\":\""+dataPacket.heartrate_class+"\", "
	"\"humidity\":\""+String(dataPacket.humidity)+"\",\"humidity_class\":\""+dataPacket.humidity_class+"\", "
	"\"oxigen\":\""+String(dataPacket.oxigen)+"\",\"oxigen_class\":\""+dataPacket.oxigen_class+"\"}";
}
#endif
...
String getDataJson(){
	...
	"\"heartrate\":\""+String(random(10,99))+"\",\"heartrate_class\":\""+getTestClass()+"\", "
	"\"humidity\":\""+String(random(10,99))+"\",\"humidity_class\":\""+getTestClass()+"\", "
	"\"oxigen\":\""+String(random(10,99))+"\",\"oxigen_class\":\""+getTestClass()+"\"}";
}
#endif
    ...
    if(lcd_update_time+500 < millis()){
        ...
        if(digitalRead(IR_PIN) == 0){
			...
			if(data_read == 1){
				...
				dataPacket.oxigen = oxigen();
				
				if(dataPacket.temperature > 102.0){
					dataPacket.message_class = "danger";
					dataPacket.message = "Fever detected!";
				}
				else if(dataPacket.humidity >= 75.0){
					dataPacket.message_class = "danger";
					dataPacket.message = "Stress detected!";
				...
				lcd.setCursor(0,0);
				lcd.print("T:");
				lcd.print(dataPacket.temperature);
				lcd.print((char) 223);
				lcd.print("F H:");
				lcd.print(dataPacket.humidity);
				lcd.print("%");
				lcd.setCursor(0,1);
				lcd.print("Ox:");
				lcd.print(dataPacket.oxigen);
				lcd.print(" Hr:");
				lcd.print(dataPacket.heartrate);
				lcd.print("");
			}
			else{
				dataPacket.message_class = "danger";
				dataPacket.message = "Smart watch";
...
String getTemplate(){
	...
	"<h1></h1>\n"
	"</div>\n"
	"<div class=\"card primary\" id=\"heartrate\">\n"
	"<span>\n"
	"<h3>Heartrate</h3>\n"
	"<svg xmlns=\"http://www.w3.org/2000/svg\" width=\"48\" height=\"48\" fill=\"white\" class=\"bi bi-water\" viewBox=\"0 0 16 16\">\n"
	...
	"<h1>30%</h1>\n"
	"</div>\n"
	"<div class=\"card primary\" id=\"oxigen\">\n"
	"<span>\n"
	"<h3>Oxygen</h3>\n"
	"<svg xmlns=\"http://www.w3.org/2000/svg\" width=\"48\" height=\"48\" fill=\"white\" class=\"bi bi-thermometer-half\" viewBox=\"0 0 16 16\">\n"
	...
	"\tdocument.getElementById(\"temperature\").children[1].innerHTML = \"\"+data.temperature+\" &deg;F\";\n"
	"\tupdateCSSClass(document.getElementById(\"temperature\"), data.temperature_class);\n"
	"\tdocument.getElementById(\"heartrate\").children[1].innerHTML = \"\"+data.heartrate+\" bpm\";\n"
	"\tupdateCSSClass(document.getElementById(\"heartrate\"), data.heartrate_class);\n"
	...
	"\tdocument.getElementById(\"oxigen\").children[1].innerHTML = \"\"+data.oxigen+\" SpO<sub>2</sub>\";\n"
	"\tupdateCSSClass(document.getElementById(\"oxigen\"), data.oxigen_class);\n"
	"}\n"
	"\n"
